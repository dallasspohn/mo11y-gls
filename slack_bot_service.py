#!/usr/bin/env python3
"""
Slack Bot Service - Systemd service wrapper for Slack bot
"""
...
import json
import logging
from slack_bot import create_slack_bot_from_config
...
def main():
    ...
    try:
        ...
        bot = create_slack_bot_from_config(agent, config_path)
        
        if not bot:
            logger.error("Failed to create Slack bot")
            logger.info("Note: Telegram is also supported as an optional alternative (see config.json)")
            sys.exit(1)
        
        # Run bot
        logger.info("Starting Slack bot...")
        bot.run(poll_interval=2)
